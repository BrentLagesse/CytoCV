...
CytoCV uses two verification patterns:

- Native email/password signup sends a numeric verification code.
- Password recovery sends a numeric verification code.
- Google and Microsoft provider sign-in uses a secure confirmation link when allauth requires email verification.

All account verification codes and provider confirmation links expire after the configured account verification window, which defaults to 5 minutes.

The provider confirmation email opens a signed link generated by the server. When the link is valid, it confirms the email address automatically and the user can continue signing in. The `Check your email` page can poll the current browser session and redirect to the dashboard once that same session is authenticated.

If the link is opened in a different browser or profile, the original tab will not auto-login because the browser sessions are separate.
