...
Frontend controller: `cytocv/core/static/js/pages/experiment.js`
...
The upload page passes endpoint URLs, upload limits, queue state, and defaults through `uploadPreparationConfig`.
Static JavaScript must read that JSON block rather than embedding Django template syntax.
...
Frontend controller: `cytocv/core/static/js/pages/pre-process.js`
...
`pre_process.html` passes UUIDs, current file index, total file count, execution mode, scale unit defaults, and selected-stat state through `preprocessPageConfig`.
...
Frontend controllers: `cytocv/core/static/js/pages/display-*.js`
...
Frontend controllers: `cytocv/core/static/js/pages/dashboard-*.js`
