...
- `GET /about/` (`about`): product and workflow overview
- `GET /about/technical/` (`about_technical`): technical architecture and methods
- `GET /about/biology/` (`about_biology`): biological context and measurements
- `GET /collaborators/` (`collaborators`): engineering and research team
- `GET /license/` (`license`): license summary and source links
- Search policy: these five routes and the homepage are the only HTML pages included in the public sitemap and marked `index,follow`

### `GET /robots.txt`

- Name: `robots_txt`
- Auth: public
- Purpose: publish crawler rules and the absolute sitemap location
- Response: UTF-8 `text/plain`

### `GET /sitemap.xml`

- Name: `sitemap`
- Auth: public
- Purpose: list the six canonical public informational URLs
- Response: XML generated by Django's sitemap framework
- Excludes: authentication, account, application, API, media, OAuth, and admin URLs
