        ...
        for route_name, url in (
            ...
            with self.subTest(route=route_name):
                ...
                for hook in (
                    ...
                    "Custom Filter changes are staged until you click Apply custom filters",
                    "Download remains unavailable until you apply or reset those changes",
                    "You can still manually check or uncheck individual statistics",
                    "Presets",
                    "Presets apply immediately. Clicking one changes the matching download columns, updates the Custom Filters below, and replaces any unapplied custom-filter changes.",
                    "Custom Filters",
                    "Custom Filters let you choose value type, contour slot, and signal pairing. Changes are staged until you click Apply custom filters, and Download remains unavailable while they are staged.",
                    "Reset changes restores the contour-intensity selection from when this window opened.",
                    "Intensity Value",
                    "Contour Slot",
                    ...
                    'role="status" aria-live="polite" aria-atomic="true" data-filter-state="applied"',
                    'data-export-intensity-action="apply" aria-controls="exportSelectionList" aria-describedby="exportIntensityFilterStatus" disabled',
    ...
    def test_export_contracts_include_puncta_source_contour_count_filter_parameter(self):
        ...
        for source in (display_source, dashboard_source):
            with self.subTest(source=source[:20]):
                ...
                self.assertIn("getCellCardFilterBadgeState", source)
                self.assertIn("prefix.textContent = badgeState.prefix", source)
                self.assertIn("separator.textContent = '\\u00b7'", source)
                self.assertIn("value.textContent = badgeState.value", source)
                self.assertIn("badge.hidden = badgeState.hidden", source)
        ...
        self.assertIn("getCellCardFilterBadgeState", results_source)
        self.assertIn("Retained cells", results_source)
        self.assertIn("Filtered view", results_source)
